...
def main():
    ...
    _ = parser.add_argument("account", nargs='?', help="Account to list (optional). Lists all account names otherwise.")
    _ = parser.add_argument("-t", "--transfer", help="Change the transfer account on the selected transactions to the one provided.")
    _ = parser.add_argument("-f", "--filter", help="Regex filter. Valid both for listing accounts and transactions.")
    _ = parser.add_argument("-d", "--duplicates", help="Provide an account and search for duplicates (same date + description)")
    ...
    if not exists(bookPath):
        raise ValueError("Provided path to GnuCash db was not found: " + bookPath)

    if inputAccount != None:
        if newTransferAccountName != None:
            changeTransferAccount(bookPath, inputAccount, newTransferAccountName, filter)
...
def changeTransferAccount(bookPath: str, inputAccount:str, newTransferAccountName: str, filter: Optional[str] = None):
    with openLedger(bookPath) as ledger:
        ...
        foundEntries = accountContainingTransactions.findEntriesWithDescription(filter)

        for e in foundEntries:
            print(e)

        user_input = input(
                "Are you sure you want to change the Transfer account of the above entries to {}? [Y/n]"
                .format(newTransferAccount.fullname)
                )
...
def unifyDuplicates(bookPath: str, mainAccountName:str, otherAccountName:str):
    with openLedger(bookPath) as ledger:
        ...
        user_input = input(
                "Are you sure you want to link the main entries to the '{}' and remove the 'other' entries? [Y/n]"
                .format(otherAccount.fullname)
                )

        if user_input.lower() == 'y':
            for mainEntry, otherEntry in mainAccount.findDuplicates(otherAccount):
...
def listTransactions(bookPath: str, accountName:str, filter: Optional[str] = None):
    with openLedger(bookPath) as ledger:
        accountToList = ledger.findAccountByName(accountName)

        print("###  Account:'{}'  filter:'{}'  ###".format(accountToList.name, str(filter)))
